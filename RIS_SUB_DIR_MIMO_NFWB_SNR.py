...
class DL_BF_TTD(nn.Module): 
    ...
    def forward(self, param_list,  param_list1, x):
        ...
        for nc in range(self.Nc):
            Phi_T[:,nc*subarray:(nc+1)*subarray] = torch.exp(1j*2*pi*fm[nc]/self.fc*Phi_phase[:,nc*subarray:(nc+1)*subarray])
...
def NFWB_RIS_VIR(param_list,param_list1, batch,Num_batchs,model_name):
    ...
    for num_batch in range(Num_batchs//Num_epochs):
        ...
        if (num_batch+1)%1==0:
            ...
            vadilate_SE = 'vadilate_R1_'+str(floor(BS_TTDs))+'BS_TTDs_'+str(floor(Nc))+'RIS_sub'+'.mat'

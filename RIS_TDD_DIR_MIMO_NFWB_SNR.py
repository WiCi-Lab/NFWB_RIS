...
class UL_CE(nn.Module): 
    ...
    def forward(self, param_list, param_list1, H_BR, H_RU, H_BU):
        ...
        for l in range(L):
            ...
            Theta1 = torch.exp(1j*self.Phase1[l,:]) # L,Nc, N_ant
...
class DL_BF_TTD(nn.Module): 
    ...
    def forward(self, param_list,  param_list1, x):
        ...
        Phi_phase = self.LN_RIS(Phi_phase)
        ...
        Phi_phase2 = self.LN_RIS(Phi_phase2)
...
def NFWB_RIS_TTD(param_list,param_list1, batch,Num_batchs,model_name):
    ...
    # BW = 7e9
    # model_name1 = './models/RIS_NFWB_TTD_debug_'+str(floor(B))+'band'+str(floor(BS_TDDs))+'BS_TTDs_'+str(floor(RIS_TDDs))+'RIS_TTDs'+str(UL)+'pilots_'+str(floor(upsnr))+'UdB'+str(floor(downsnr))+'DdB'+'.pth'
    ...
    for num_batch in range(Num_batchs//Num_epochs):
        ...
        #     RU = np.linspace(5,25,5)
        #     RU_index = random.randint(0, len(RU)-1)            
        #     R = RU[RU_index]
        ...
        if (num_batch+1)%1==0:
            ...
            train_SE = 0
            start = datetime.datetime.now()
            vadilate_SE = '/vadilate_Dis_'+str(floor(BS_TDDs))+'BS_TTDs_'+str(floor(RIS_TDDs))+'RIS_TTDs'+'.mat'
...
def NFWB_RIS_TTD_test(param_list,param_list1, batch,Num_batchs,model_name):
    ...
    # H_RU = torch.tensor(sio.loadmat('H_RU'+str(math.floor(B))+'band'+str(math.floor(N_ant))+'elements'+str(ts)+'spacing'+str(R)+'m'+'.mat')['H_RU']).cuda()
    # H_BU = torch.tensor(sio.loadmat('H_BU'+str(math.floor(B))+'band'+str(math.floor(N_ant))+'elements'+str(ts)+'spacing'+str(R)+'m'+'.mat')['H_BU']).cuda()
    # H_BR = torch.tensor(sio.loadmat('H_BR'+str(math.floor(B))+'band'+str(math.floor(N_ant))+'elements'+str(ts)+'spacing'+str(R)+'m'+'.mat')['H_BR']).cuda()

...
def Channel_BS_RIS_LOS_PLA(param_list,flag):
    ...
    for nc in range(Nc):
        ...
        if flag ==1:
            ...
            beta_pk = sf*(torch.randn(1)+1j*torch.randn(1))/sqrt(2)
        elif flag ==2:
    ...
    for ib in range(batch):
        ...
        for nc in range(Nc):
            ...
            if flag ==1:
                ...
                RP = 5
                tau = RP/c
                beta_pk = c/(4*pi*fm[nc]*RP)*torch.exp(-1/2*absorption*RP)*torch.exp(-1j*2*pi*tau*fm[nc])
...
if  __name__ == '__main__':
    ...
    sio.savemat('H_RU2'+str(math.floor(BW))+'band'+str(math.floor(N))+'elements'+str(ts)+'spacing'+str(R_RU)+'m'+'.mat', {'H_RU': H_RU.cpu().detach().numpy()})
    sio.savemat('H_BU2'+str(math.floor(BW))+'band'+str(math.floor(N))+'elements'+str(ts)+'spacing'+str(R_RU)+'m'+'.mat', {'H_BU': H_BU.cpu().detach().numpy()})
    sio.savemat('H_BR2'+str(math.floor(BW))+'band'+str(math.floor(N))+'elements'+str(ts)+'spacing'+str(R_RU)+'m'+'.mat', {'H_BR': H_BR.cpu().detach().numpy()})

...
class UL_CE(nn.Module): 
    def __init__(self, param_list, param_list1): 
        ...
        # self.S_digital = torch.nn.Parameter(torch.randn(L,Nc,RF_chain,Nr)+1j*torch.randn(L,Nc,RF_chain, Nr)).cuda() #L个时隙BS导频,这里假设频率选择
    ...
    def forward(self, param_list, param_list1, H_BR, H_RU, H_BU):
        ...
        for l in range(L):
            ...
            Theta1 = self.Phase1[l,:] # L,Nc, N_ant
            # Theta2 = torch.exp(1j*self.Phase2[l,:]) # L,Nc, N_ant
            # Phase_TDD = torch.exp(1j*self.Phase_TDD[l,:]) # L,Nc, N_ant
            S_analog = torch.exp(1j*self.S_analog[l,:,:,:])/sqrt(self.M_ant)
            Phi_T = torch.zeros(self.N_ant).cuda() + 1j
            
            subarray = self.N_ant//self.Nc
            
            for nc in range(self.Nc):
                Phi_T[nc*subarray:(nc+1)*subarray] = torch.exp(1j*2*pi*fm[nc]/self.fc*Theta1[nc*subarray:(nc+1)*subarray])
            ...
            for nc in range(self.Nc):
                ...
                Theta[nc,:,:] = torch.diag_embed(Phi_T)
...
class DL_BF_TTD(nn.Module): 
    ...
    def forward(self, param_list,  param_list1, x):
        ...
        Phi_phase = self.LN_RIS(Phi_phase).reshape(-1,self.N_ant)
        
        # Phi_phase = 2*pi * self.sig(Phi_phase)
        seg = (2*pi)/(2**bit)
        # # seg = (1/2**bit)
        # # data_normalized1 = 2*pi*Phi_phase
        # Phi_phase_BIT = torch.floor(Phi_phase/seg)* seg
        
        
        # Phi_phase = self.trans_RIS1(Phi_phase)
        # Phi_phase = torch.exp(1j*Phi_phase.reshape(-1,self.N_ant))
        ...
        Phi_T = torch.zeros(Phi_phase.shape[0],self.N_ant).cuda() + 1j
        for nc in range(self.Nc):
            data_normalized1 = self.sig(fm[nc]/self.fc*Phi_phase[:,nc*subarray:(nc+1)*subarray])
            # data_normalized1 = torch.clamp(Phi_phase[:,nc*subarray:(nc+1)*subarray],-1*pi, pi)
            data_normalized1 = 2*pi*data_normalized1 
            Phi_phase_BIT =   torch.floor(data_normalized1/seg)* seg
            Phi_T[:,nc*subarray:(nc+1)*subarray] = torch.exp(1j*Phi_phase_BIT)
            # Phi_T[:,nc*subarray:(nc+1)*subarray] = torch.exp(1j*2*pi*fm[nc]/self.fc*Phi_phase_BIT[:,nc*subarray:(nc+1)*subarray])

        
        
        Phi_VIR = torch.diag_embed(Phi_T) 
        
        Phi_WB = torch.zeros(Phi_phase.shape[0],self.Nc,self.N_ant,self.N_ant).cuda() + 1j
        # for nc in range(Nc):
        ...
        F_RF_TTDs = torch.zeros(Phi_phase.shape[0],self.Nc,self.M_ant,self.RF_chain).cuda() + 1j
        for nc in range(self.Nc):
        ...
        return Phi_WB,F_RF_TTDs,F_BB_SDMA,Phi_phase  #Phi[batch,1,M_ant,M_ant]  F_RF[batch,1,M_ant,K]  F_BB_SDMA[batch,Nc,K,K] F_BB_RS[batch,Nc,K,1] 
...
def NFWB_RIS_VIR(param_list,param_list1, batch,Num_batchs,model_name):
    ...
    if TL:
        pre_model_name = 'RIS_NFWB_VIR_debug_' + str(floor(B)) + 'band' + str(
            floor(BS_TTDs)) + 'BS_TTDs_' + str(UL) + 'pilots_' + str(floor(upsnr)) + 'UdB' + str(
    ...
    # BW = 7e9
    # model_name1 = './models/RIS_NFWB_VIR_'+str(floor(BW))+'band'+str(floor(BS_TTDs))+'BS_TTDs_'+str(UL)+'pilots_'+str(floor(upsnr))+'UdB'+str(floor(downsnr))+'DdB'+'.pth'

    # model = torch.load(model_name) 
            
    # optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()),lr=0.001)
    # optimizer = torch.optim.SGD(net_AE.parameters(), lr=0.0001, momentum=0.9, weight_decay=5e-4,nesterov=True)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.001, betas=(0.9, 0.999), eps=1e-07,weight_decay=1e-5)
    ...
    for num_batch in range(Num_batchs//Num_epochs):
        ...
        lr = adjust_learning_rate(optimizer, num_batch, 5e-5, 1e-5, Num_batchs//Num_epochs)
        ...
        for num_epoch in range(Num_epochs):
            ...
            R_sum, Loss = loss_func(param_list, param_list1, H_BR,H_RU1,H_BU,Phi,F_RF,F_BB_SDMA,F_BB_RS, train_snr[snr_index])
            loss = -R_sum
            ...
            # print('num_epoch:',num_epoch,'loss',Loss.detach_(),'SE',R_sum.detach_()) 
        ...
        if (num_batch+1)%1==0:
            ...
            vadilate_SE = './results/vadilate'+str(floor(BS_TTDs))+'BS_TTDs_'+str(floor(Nc))+'RIS_sub'+'.mat'
...
def NFWB_RIS_VIR_test(param_list,param_list1, batch,Num_batchs,model_name):
...
#                 # H_BR = (Channel_BS_RIS_LOS(param_list,flag)+Channel_BS_RIS_NLOS(param_list,flag))
#             else:
#                 H_RU1 = sqrt(Gr_gain)*Channel_RIS_UE_MIMO_NLOS(param_list,batch,flag)
#                 H_BU = sqrt(Gt_gain*Gr_gain)*Channel_BS_UE_MIMO_NLOS(param_list,batch,flag)
#                 H_BU = H_BU.reshape(batch,K,Nc,Nr,M_ant) 
#                 H_RU1 = H_RU1.reshape(batch,K,Nc,Nr,N_ant) 
#                 H_BR = sqrt(Gt_gain)*(Channel_BS_RIS_LOS(param_list,flag)+Channel_BS_RIS_NLOS(param_list,flag)) 
